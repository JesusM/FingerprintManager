...
class EncryptionManagerTest : BaseTest() {
    ...
    fun notAvailableIfHardwareNotPresent() {
        ...
        val encryptionCallback = Mockito.mock<JFingerprintManager.EncryptionCallback>(JFingerprintManager.EncryptionCallback::class.java)

        jFingerprintManager.encrypt("message", encryptionCallback, mockFragmentManager)
        verify<JFingerprintManager.EncryptionCallback>(encryptionCallback).onFingerprintNotAvailable()
        verify<JFingerprintManager.EncryptionCallback>(encryptionCallback, never()).onEncryptionSuccess(any())
    }
    ...
    fun decryptionFailIfMessageEmpty() {
        ...
        val callbackAdapter = Mockito.mock<JFingerprintManager.DecryptionCallback>(JFingerprintManager.DecryptionCallback::class.java)
        jFingerprintManager.decrypt("", callbackAdapter, mockFragmentManager)

        verify<JFingerprintManager.DecryptionCallback>(callbackAdapter).onDecryptionFailed()
        verify(mockSystem, never()).showDialog()
    }
    ...
    private inner class EncryptionCallbackAdapter : JFingerprintManager.EncryptionCallback {
